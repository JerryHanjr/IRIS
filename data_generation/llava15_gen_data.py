"""
On-policy Self-Generation (Eq. 7)
Sample K diverse candidate responses from current policy: {y_k} ~ pi_theta(y|v,x)
"""
import os
import io
import tqdm
...
def llava15_qa_colloator_fn(data_list, tokenizer, image_processor, config):
    ...
    if image_processor is None:
        # Try to lazily construct the image processor from config
        try:
            from transformers import AutoImageProcessor
            vision_id = getattr(config, 'mm_vision_tower', None)
            if vision_id:
                print(f"image_processor is None in collator; trying AutoImageProcessor.from_pretrained({vision_id}) ...")
            ...
            try:
                ...
                vision_id = getattr(config, 'mm_vision_tower', None)
                if vision_id:
                    print(f"AutoImageProcessor failed ({e_auto}); trying CLIPImageProcessor.from_pretrained({vision_id}) ...")
                    image_processor = CLIPImageProcessor.from_pretrained(vision_id)
            except Exception as e_clip:
                raise RuntimeError(
                    "image_processor is None. The vision tower was not properly initialized. "
                    "This usually happens when the model checkpoint is incomplete or corrupted. "
                    "Please verify that:\n"
                    "1. The model checkpoint contains vision tower weights\n"
                    "2. The model config has 'mm_vision_tower' specified\n"
                    "3. The vision tower model files are accessible\n"
                    f"Details: AutoImageProcessor error: {e_auto}; CLIPImageProcessor error: {e_clip}"
                )
